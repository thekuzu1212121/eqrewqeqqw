...
exports.run = async (client, message, args) => {
  let frenzy_ibrahim =
    (await db.fetch(`Frenzy?Code?Ototag_${message.guild.id}`)) ||
    (await db.fetch(`Frenzy?Code?OtotagKanal_${message.guild.id}`));
  if (frenzy_ibrahim)
    return message.reply(
      `:x: Bu sistem zaten aktif durumda. Kapatmak için **${prefix}ototagkapat**`
    );
  let frenzy_kanal = message.mentions.channels.first();
  let frenzy_tag = args.slice(1).join(" ");
  if (!frenzy_kanal || !frenzy_tag)
    return message.reply(
      `Ototag vermek için; **t!ototag #kanal @rol**`
    );
