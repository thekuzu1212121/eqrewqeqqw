...
exports.run = async (client, msg, args) => {
  if (!args[0]) {
    return msg.channel.send(
      "Lütfen seçimini yap taş, kağıt yada makas & \nt!tkm <`taş,kağıt,makas`>"
    );
  }
  ...
  if (!rps.includes(choice)) {
    return msg.channel.send(
      "Lütfen seçimini yap taş, kağıt yada makas & tkm t!`t,k,m>`"
    );
  }
  ...
  if (result === "won") {
    ...
    answer =
      ":neutral_face: Sonuç **Berabere** :neutral_face:\nSenin Seçimin: `" +
      choice +
      "` | Bot'un Seçimi: `" +
      botChoice +
      "`";
